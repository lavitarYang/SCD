...
def main(args):
    ...
    while(cap.isOpened()):
        ...
        if ret:
            ...
            if len(outputs[0]['boxes']) != 0:
                ...
                # with open("/content/drive/MyDrive/fasterRcnn/log.txt",'wb+') as f:
                  # f.writelines(frame_count/cv_fps)
    ...
    # after release video io 
    # we'll open a file io :P lo log json file
    # (1) result (I'm mean location)
    # (2) timestamp
    
    # with open('/content/drive/MyDrive/cloud_demo/render_data.json','r+') as inf:

      # json_object=json.load(inf)
      # python_object=json.loads(json_object)
       
      # json_object=json.loads(inf)
      # print(json)
      # DICT = dict()
      # FILE=f"{OUT_DIR}/{save_name}.mp4"
      # DICT["source"]=FILE
      # DICT["timestamp"]=TIMDICT
      # LIST = [DICT]
      # dict is already json file don't double serialize it
      # json_object=json.dumps(LIST)

      # json.dump(LIST,inf)

    # Calculate and print the average FPS
    # this help me tracking I using all of
    # my computation 
